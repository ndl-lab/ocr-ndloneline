...
## 1.概要
国立国会図書館デジタルコレクション(https://dl.ndl.go.jp)からインターネット公開している著作権保護期間の満了したデジタル資料のうち、
...
正解テキストデータは本リポジトリのaozora_metadata.csvを参照してください。

## 正解データの形式について
各行タブ区切りに、左から「ファイル名」「正解テキストデータ」「向き(tateまたはyoko)」「切り出し画像のURL(IIIF Image API)」の情報が記載されています。
